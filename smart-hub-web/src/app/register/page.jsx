...
function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
        const restCheckUser = await fetch("http://localhost:3000/api/checkUser", {
          method: 'POST',
          headers: {
        ...
        const { user } = await restCheckUser.json()
  ...
  return (
    <div>
        ...
        <div className='container mx-auto py-5'>
            ...
            <hr className="my-3" />
            <p>Have an account? go to <Link className='text-blue-500 hover:underline' href="/login">Login</Link> Page</p>
        </div>
    </div>
  )
}
