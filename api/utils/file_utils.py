...
# post meter to FILE_PATH = os.path.join(BASE_DIR, "mock_data", "posted")
def post_meter_data(meter_id: str, index: int):
    try:
        IDENTIFIER = meter_id
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        FILE_PATH = os.path.join(BASE_DIR, "mock_data", "individuals")
        POSTED_PATH = os.path.join(BASE_DIR, "mock_data", "posted")
        FILENAME = f"{meter_id}.csv"
        ...
        # Write the JSON data to a new file only index
        with open(os.path.join(POSTED_PATH, f"{meter_id}.json"), 'w') as file:
